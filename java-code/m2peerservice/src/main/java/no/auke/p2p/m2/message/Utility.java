 ...
 */
package no.auke.p2p.m2.message;
public class Utility {
	...
	public static final byte[] longToEightBytes(long value) throws UtilityException {
	...
	public static final long eightBytesToLong(byte[] value) throws UtilityException {
		if (value.length < 8) {
			throw new UtilityException("Byte array too short!");
		}
		int temp0 = value[0] & 0xFF;
		...
		int temp6 = value[6] & 0xFF;
		int temp7 = value[7] & 0xFF;
		return (((long) temp0 << 56) + ((long) temp1 << 48) + ((long) temp2 << 40) + ((long) temp3 << 32) + ((long) temp4 << 24) + (temp5 << 16) + (temp6 << 8) + temp7);
	}
}
